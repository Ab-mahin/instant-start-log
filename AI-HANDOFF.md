...
│   ├── movie.go                     # Parent: mahin movie (registers all subcommands)
...
Database location: `~/movie-cli-output/mahin.db`  
...
| `~/movie-cli-output/` data dir | Single location for all data, easy to backup/migrate |
...
- Specifications: `SPEC.md` (root) + `spec/01-app/` (app spec) + `spec/02-app/issues/` (issue write-ups)
...
1. Create `spec/02-app/issues/XX-{slug}.md` using the template
...
make build-windows   # Cross-compile → mahin.exe (amd64)
make build-mac-arm   # Cross-compile → mahin-darwin-arm64
make build-mac-intel # Cross-compile → mahin-darwin-amd64
make build-linux     # Cross-compile → mahin-linux-amd64
...
### Version Injection
```bash
go build -ldflags="-X version.Version=1.0.0 -X version.Commit=$(git rev-parse --short HEAD) -X version.BuildDate=$(date -u +%Y-%m-%dT%H:%M:%SZ)" -o mahin .
```
...
Set via: `mahin movie config set <key> <value>`  
Get via: `mahin movie config get <key>`
...
| 8 | Compiled `mahin` binary committed to repo | Deleted binaries from repo |
...
| `spec/02-app/issues/00-issue-template.md` | Template for new issue write-ups |
| `spec/02-app/issues/01-05` | Individual issue write-ups |
...
# 3. Configure
./mahin movie config set tmdb_api_key YOUR_API_KEY
./mahin movie config set movies_dir ~/Movies
./mahin movie config set scan_dir ~/Downloads

# 4. Use it
./mahin movie scan ~/Downloads      # Scan for videos
./mahin movie ls                    # List library
./mahin movie search "Inception"    # Search TMDb
./mahin movie info 1                # View details by ID
./mahin movie info "The Matrix"     # View details by title
./mahin movie move                  # Interactive file move
./mahin movie suggest 5             # Get 5 recommendations
./mahin movie stats                 # Library statistics
```

---

*This file is the single source of truth for any AI continuing work on mahin-cli-v1. If you've read this, you're up to speed.*
