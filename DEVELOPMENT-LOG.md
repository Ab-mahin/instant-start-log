# mahin-cli — Development Log

> **Project**: mahin-cli-v1  
> **Language**: Go 1.22  
...
- Database stored at `~/movie-cli-output/mahin.db` with WAL mode
- Thumbnails downloaded to `~/movie-cli-output/thumbnails/`
- Move history logged to `~/movie-cli-output/json/history/` as JSON
...
> **User**: mahin movie ls will only show my local libraries' movies and TV shows, not the databases. Read this and find out if these things are done or not.
...
> **User**: With this "mahin movie move" command, I first want to choose a directory on my local machine. When I select a local directory, it shows all the movie or TV show files in it, and then I can perform a move operation.

**What Was Done**:
- Completely rewrote `cmd/movie_move.go` to a "browse-first" workflow:
  1. **Pick source directory** — choose Downloads, configured `scan_dir`, or enter custom path (or pass as argument: `mahin movie move ~/Videos`)
...
> **User**: Read full project, if anything makes you annoyed or irrelevant then inform me. I want to create mahin CLI, does it stay on its focus?

**What Was Done**:
- Read every file in the project
- **Confirmed**: Project is 100% focused on mahin CLI movie management — no scope creep
- **Found 8 hygiene issues**:
  ...
  8. Compiled `mahin` binary committed to repo
...
| 8 | Binary committed to repo | Deleted `mahin` and `mahin-cli-v1` binaries | `mahin`, `mahin-cli-v1` (deleted) |

**Files Modified**: `go.mod`, `db/helpers.go`, `main.go`, `cmd/movie_scan.go`, `cmd/movie_info.go`  
**Files Deleted**: `readme.txt`, `mahin`, `mahin-cli-v1`  
...
- Found and deleted one additional binary (`mahin-cli-v1`) that was still in the repo
...
- Rewrote `README.md` with comprehensive documentation:
  ...
  - Data storage layout (`~/movie-cli-output/`)
...
- **Data directory**: `~/movie-cli-output/` (DB, thumbnails, JSON history)
