...
`mahin` is a cross-platform CLI tool for managing a personal movie and TV show library. It:
...
│   ├── movie.go               # mahin movie (parent, registers subcommands)
│   ├── movie_config.go        # mahin movie config [get|set]
...
All data resides in `~/movie-cli-output/`:
...
**Args**: None  
**Output**: `👋 Hello from mahin-cli-v1!` + version string  
...
6. For each video file:
   ...
   - If API key available:
     ...
     - Download poster → `~/movie-cli-output/thumbnails/<slug>/<slug>.jpg`
...
**Purpose**: Paginated list of all locally indexed media.  
**Args**: None  
**IMPORTANT**: Only shows items indexed via `scan` — NOT from `search` or `info` results that lack files.  
**Behavior**:
1. Read `page_size` from config (default 20)
2. Count total media
3. Display page with: number, clean title, year, rating (TMDb→IMDb fallback), type icon
4. Interactive navigation: `N`=next, `P`=prev, `Q`=quit, number=view detail
5. Detail view: full metadata card with title, year, type, ratings, genres, director, cast, description, thumbnail path, file path
6. Clears terminal screen (`\033[H\033[2J`) between pages

### 4.7 `mahin movie search <name>`
...
### 4.10 `mahin movie move [directory]`
...
9. **Track in database**:
   ...
   - Append to JSON history file: `~/movie-cli-output/json/history/<slug>/move-log.json`
10. Print success message

### 4.11 `mahin movie rename`
...
### 4.12 `mahin movie undo`

**Purpose**: Revert the most recent move/rename operation.  
**Args**: None  
**Behavior**:
1. `GetLastMove()` — latest `move_history` record where `undone=0`
2. Verify file exists at `to_path`
3. `os.Rename(toPath, fromPath)` — move back
4. Mark record as `undone=1` in DB
5. Update media `current_file_path` back to `from_path`
6. **NOTE**: No confirmation prompt currently. Can be called repeatedly to undo multiple moves.

### 4.13 `mahin movie play <id>`
...
### 4.14 `mahin movie stats`

**Purpose**: Display library statistics.  
**Args**: None  
**Output**:
1. Counts: total movies, total TV shows, total
2. Top 10 genres with visual bar chart (`█` characters, max 30 width)
3. Average IMDb rating (if available)
4. Average TMDb rating (if available)
**Note**: Loads all media (up to 10,000) to compute averages.
...
| Target | Description |
|---|---|
| `make build` | Build for current OS → `./mahin` |
| `make build-windows` | Cross-compile → `mahin.exe` |
| `make build-mac-arm` | macOS ARM64 → `mahin-darwin-arm64` |
| `make build-mac-intel` | macOS AMD64 → `mahin-darwin-amd64` |
...
```
-ldflags "-s -w
  -X github.com/mahin/mahin-cli-v1/version.Version=<git tag>
  -X github.com/mahin/mahin-cli-v1/version.Commit=<short SHA>
  -X github.com/mahin/mahin-cli-v1/version.BuildDate=<YYYY-MM-DD>"
```
