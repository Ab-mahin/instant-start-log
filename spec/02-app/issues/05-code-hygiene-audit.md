...
2. **Where**: `db/helpers.go`, `main.go`, `cmd/movie_scan.go`, `cmd/movie_info.go`, `cmd/movie_resolve.go`, `readm.txt`/`readme.txt`, `go.mod`, root `mahin` binary.
...
8. **`mahin` binary**: Deleted committed binary from repo root.
...
3. **Spec references**: `spec/01-app/01-project-spec.md` (DRY, file structure rules).
